...
export class Trade extends Card{
    ...
    draw(isUpdate=false){
        const that = this;
        const {trade,$container,auth} = this;
        const profit = parseFloat(trade.profit) + parseFloat(((trade.type=='forex')?0:trade.amount));
        const profitPercent = profit/parseFloat(trade.invested);
        console.debug('trade render',trade);
        // if(!isUpdate)
        $container.html('');
        ...
        if(trade.status_id!=20  && trade.multiplier >=50){
