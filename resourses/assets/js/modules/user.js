    ...
    list(container, d, x, s) {
        ...
        for (var i in d.data) {
            ...
            if (showBalances) {
                ...
                tr.append('<td class="ui aligned center">' + (((crm.user.getMeta(row.meta, 'verified')) == "true") ?'<i class="icon checkmark"></i>' : '') +'</td>');
                tr.append('<td class="ui aligned center">' + (((crm.user.getMeta(row.meta, 'can_trade')) == "true") ?'<i class="icon checkmark"></i>' : '') +'</td>');
                var s = ''
                for (var i = 0; i < 2; ++i) {s += (row.accounts[i] == undefined) ? '' : row.accounts[i].type + '<span class="ui right">' + currency.value(row.accounts[i].amount, "USD") + '</span><br>';}
                tr.append('<td class="aligned right">' + s + '</td>');
                tr.append('<td>' + row.status.title + '</td>');
            }
            else {
                var str = '';
                for (var a in row.accounts) {
                    if (row.accounts[a].type == 'real') {
                        str = row.accounts[a].amount.currency('USD', 2);
                    }
                }
                tr.append('<td class="ui right aligned">' + row.balance.currency('USD',2) + '</td>');
            }

            let kyc = crm.user.getMeta(row.meta, 'kyc');
            // //console.debug(`user ${row.id} kyc = ${kyc}`,row.meta);
            switch (kyc) {
                case "true":
                case "2":
            ...
            tr.append('<td>' + (row.rights?row.rights.title:'') + '<br/><small>' + (row.status?row.status.title:'') + '</small>' + '<br/>KYC: ' + kyc + '</td>');
            ...
            tr.append('<td>' + crm.user.showManager(row.manager)
