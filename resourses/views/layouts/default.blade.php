...
<head>
    ...
    <!--  Vendor files -->
    <base href="/"/>
    <link rel="stylesheet" href="{{ asset('css/icon.css') }}?{{ filesize('css/icon.css') }}">
    ...
    <!-- <link rel="stylesheet" href="{{ asset('chart2/chart.css') }}"> -->
...
<body class="{{ mb_strtolower(config('app.name', 'Laravel')) }}" style="height: 100%;">
    ...
    {{-- <script src="{{ asset('vendor/jquery.toastmessage.js') }}?{{filesize('vendor/jquery.toastmessage.js')}}"></script> --}}
    ...
    <script>
        ...
        @foreach($events as $event)
            try{
                ...
                console.warn('blade events error',e);
            }
        @endforeach
        window.crm.events.init(events);
    </script>
    @stack('scripts')
</body>
</html>
