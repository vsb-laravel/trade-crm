...
export class User extends Card{
    ...
    load(){
        ...
        return new Promise((resolve,reject)=>{
            if(user.transactions) {resolve();}
            if(user.deal) resolve();
            if(user.trades) resolve();
            $.ajax({
                url:`/user/${user.id}`,
                beforeSend:(e)=>{
                    console.log(`User ${user.id} load data`);
                },
                success:(d)=>{
                    $.extend(user,d);
                    // console.debug('loaded user ',user, newuser);
                    // that.user = newuser;
                    resolve();
                },
                complete:(d)=>{},
    ...
    fresh(d){
        if(!this.compare(d)){
            ...
            this.user = d;
            try{
                // if(this.$container.visibility('is on screen') ){
                if(this.$container.isVisible()){
                    this.rendered = false;
                    this.shouldUpdate = true;
                    this.isUpdate = true;
                    this.draw(true);
                }else console.debug('card is off screen');
            }
            catch(e){
    ...
    draw(isUpdate=false){
        ...
        let $grid = isUpdate?this.$container.find('.grid:first'):$(`<div class="ui stackable grid"></div>`).appendTo($content);
            // $grid = isUpdate?this.$container.find('.row:first'):$(`<div class="ui row"></div>`).appendTo($grid);
        ...
        if(!isUpdate){
            this.load().then(()=>{
                ...
                let $tab = isUpdate?$right.find('.tabular.menu:first'):$(`<div class="ui top attached tabular stackable menu"></div>`).appendTo($right);
                ...
                if(auth.rights_id==10 || user.rights_id==1) this.finance($right.find(`.tab[data-tab="finance_${user.id}"]`).length?$right.find(`.tab[data-tab="finance_${user.id}"]`): $(`<div class="ui bottom attached tab segment" data-tab="finance_${user.id}"></div>`).appendTo($right))
            ...
            if(auth.rights_id==10 || user.rights_id==1) this.finance(this.$container.find(`.right-column .tab[data-tab="finance_${user.id}"]`))
    ...
    leftColumn($div,isUpdate=false){
        ...
        if(ftd)$div.append(`<div class="ui clearing"><label>${__('crm.customers.ftd')}</label><h5 class="ui right floated header" style="display:inline;">${ftd.manager.title}</h5></div>`)
        if(user.source)$div.append(`<div class="ui clearing item"><label>${__('crm.customers.source')}</label><h5 class="ui right floated header" style="display:inline;">${user.source}</h5></div>`);
        $(`<div class="ui equal width form submiter globe" data-action="/json/user/${user.id}/update" data-callback="crm.user.touch" id="">
            <div class="field">
        ...
        </div>`).appendTo($div);//.dropdown().dropdown('set value', user.status_id);
        let $form=null;
        if(this.auth.can.kyc){
            $div.append(`<div class="ui horizontal divider">${__('crm.options.admintree')}</div>`)
            $form=$(`<div class="ui form submiter globe" data-action="/json/user/${user.id}/update" data-callback="crm.user.touch" id=""></div>`).appendTo($div);
            if(user.rights_id>1)$(`<div class="field">
        ...
        $form.append(`<div class="ui horizontal divider">${__('crm.customers.access')}</div>`)
        $(`<div class="ui field"><label>${__('crm.customers.login')}</label></div>`).appendTo($form).append(new VUICopytext(user.email))
    ...
    kyc($div){
        const {user,auth} = this;
        $div.html('');
        const $kyc = $(`<div class="ui form"><div class="ui fields three wide"></div></div>`).appendTo($div).find('.fields');
        if(this.auth.can.kyc){
            ...
            $(`<div class="field submiter" data-action="/json/user/meta?meta_name=kyc&user_id=${user.id}" data-name="user-kyc" data-callback="crm.user.touch">
                <input type="hidden" class="submit"/>
                <label>KYC</label>
                <div class="ui search selection dropdown">
                    <input type="hidden" data-name="meta_value" value="${kyc?kyc:0}" onchange="$(this).parent().parent().find('.submit').click()"/>
                    <i class="dropdown icon"></i>
                    <div class="default text"></div>
                    <div class="menu" tabindex="-1">
            ...
            $(`<div class="ui field">
                ...
                <div class="ui slider checkbox submiter" data-action="/json/user/meta?meta_name=can_trade" data-name="user-can-trade" data-callback="userCanTrade">
                    ...
                    <label>${(can_trade=="1")?__('crm.on'):__('crm.off')}</label>
                </div>
            </div>`).appendTo($kyc);
            $(`<div class="ui field">
                <label>${__('crm.customers.email_verified')}</label>
                <div class="ui slider checkbox submiter" data-action="/json/user/${user.id}/update" data-callback="crm.user.touch" id="">
                    <input type="hidden" data-name="user_id" value="${user.id}" />
                    <input type="checkbox" class="submit" data-trigger="change" data-name="email_verified" ${(user.email_verified=="1")?'checked="checked" value="true"':''} />
                    <label>${(user.email_verified=="1")?__('crm.on'):__('crm.off')}</label>
                </div>
            </div>`).appendTo($kyc);
        }
        $div.append(`<div class="ui horizontal divider">${__('crm.customers.name')}</div>`)
        ...
        <div class="ui form">
            <div class="ui fields submiter" data-action="/json/user/meta?meta_name=pairgroup">
                <div class="field four wide">
                    <input type="hidden" data-name="user_id" value="${user.id}" />
                    <input type="hidden" id="user_${user.id}_pair_group_submit" class="submit"/>
                    <label>${__('crm.instruments.groups.title')}</label>
                    <div class="ui selection dropdown loadering" id="user_${user.id}_pair_group" data-name="meta_value" data-value="${this.getMeta('pairgroup') || ''}" data-action="/pairgroup" data-autostart="true" data-trigger="" data-function-change="$('#user_${user.id}_pair_group_submit').click()"></div>
                </div>
            </div>
            <div class="ui fields submiter" data-action="/json/user/meta?meta_name=margincall">
                <div class="field four wide">
                    <input type="hidden" data-name="user_id" value="${user.id}" />
                    <label>${__('forex.marginCall')}</label>
                    <div class="ui input"><input type="number" id="user_${user.id}_margincall" data-name="meta_value" value="${user.margincall || ''}"/></div>
                </div>
                <div class="field">
                    <label>&nbsp;</label>
                    <div class="ui green icon button submit"><i class="ui checkmark icon"></i> ${__('crm.save')}</div>
                </div>
            </div>
            <div class="ui fields submiter" data-action="/json/user/meta?meta_name=stopout">
                <input type="hidden" data-name="user_id" value="${user.id}" />
                <div class="field four wide">
                    <label>${__('forex.stopOut')}</label>
                    <div class="ui input"><input type="number" id="user_${user.id}_stopOut" data-name="meta_value" value="${user.stopout || ''}"/></div>
                </div>
                <div class="field">
                    <label>&nbsp;</label>
                    <div class="ui green icon button submit"><i class="ui checkmark icon"></i> ${__('crm.save')}</div>
    ...
    finance($div){
        ...
        let $cols = $div.find(`#user_${user.id}_card_finance_grid_row`);
        $cols = $cols.length?$cols:$(`<div id="user_${user.id}_card_finance_grid" class="ui stackable grid two columns"><div id="user_${user.id}_card_finance_grid_row" class="ui row"></div></div>`).appendTo($div).find(`#user_${user.id}_card_finance_grid_row`);
        let $col = $cols.find(`#user_${user.id}_card_finance_grid_totals`);
        $col = $col.length?$col:$(`<div class="ui column" id="user_${user.id}_card_finance_grid_totals"></div>`).appendTo($cols);
        $col.html(`<div class="ui horizontal divider">${__('crm.user.totals')}</div>
        ...
        if(auth.can.deposit){
            ...
            const accounts = new Collection(realAccs,{name:'currency.name',value:'id',desc:'currency.code'});
            ...
            window[f_success]=(response,container,request)=>{
                ...
                $(`#user_${user.id}_card_finance_grid_deposit input`).val();
            }
            ...
            $col = $cols.find(`#user_${user.id}_card_finance_grid_deposit`);
            $col = $col.length?$col:$(`<div class="ui column" id="user_${user.id}_card_finance_grid_deposit">
        ...
        if($div.find(`#user_${user.id}_card_finance_grid_transactions`).length) skymechanics.touch(`user-finance-${user.id}`);
        else {
            $(`<div id="user_${user.id}_card_finance_grid_transactions" class="ui horizontal divider">${__('crm.finance.transactions')}</div>`).appendTo($div);
