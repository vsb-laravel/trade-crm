...
window.skymechanics={
    ...
    reload:function(){
        ...
        $(".slider.checkbox:not(.slider-checkbox-assgined)").each(function(){
            $(this).checkbox({
                onChange:function(e){
                    const $checker = $(this);
                    const $that = $checker.parent('.slider.checkbox')
                    const $label = $that.find('label');
                    console.debug('slider ',$that,$label,$that.checkbox('is checked'));
                    $that.checkbox('is checked')?$label.html(__('crm.on')):$label.html(__('crm.off'));
                }
            });
        }).addClass("slider-checkbox-assgined");
    ...
    needTouch:function(uid){
        var obj = (arguments.length>1)?arguments[1]:'loader';
        switch(obj){
            case 'loader':
                if(skymechanics._loaders[uid] )skymechanics._loaders[uid].needTouch();
                break;
            default: reject("No loader found",uid);break;
        }
    },
...
window.page={
    ...
    show:function(){
        ...
        if(window.menuBar)window.menuBar.sidebar("hide");
        $('#main_dimmer').fadeOut();
...
// $(document).ready(function(){
    ...
    // $('.ui.sidebar').sidebar();
    window.menuBar = $('.sidebar#menuBar')
        .sidebar({
            mobileTransition:'overlay',
            duration: 128,
            dimPage:false,
